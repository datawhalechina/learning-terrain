...
## 附录 5.B：真实 LLM 上的直接证据——信念锚点与证据动力学

§5.A 的数值实验验证的是公式内部自洽：动力学由闭式解的欧拉步构造，必然贴闭式解。本节给出**真实 LLM 上的直接证据**：把 5.8 节的推理算子 $F: \mathcal{P} \to \mathcal{P}$ 用真实模型（DeepSeek-V4-flash，推理型）实例化，逐步注入证据、逐步读取信念，逐项检验永霖公式与永霖极限在真实推理行为上的对应物。完整源码：`docs/public/scripts/yonglin_llm_exp.py`（锚点与证据轨道）、`docs/public/scripts/compress_phase.py`（压缩与记忆检验），图见 `ch05_llm_evidence.png`。

**实验设置**。三色袋世界，仍用信念单纯形 $\Delta^2$。袋内球比例 $p^*$ 取三档，含全书基准 $(0.70, 0.20, 0.10)$（$\kappa = 7.00$）；已知可信度 $d \in \{0.90, 0.70, 0.55\}$ 的检测仪每步对抽出的球做一次带噪声颜色报告（报告似然 $P(r|c)$ 精确已知，真后验 $p^B_t$ 逐步闭式计算）；证据序列即检测报告序列 $T = 6$ 条。模型每步看到的输入是"袋参数 + 此前 $t$ 条报告 + 强制一语回答"四项，信念由**最终答案 token 的 top-k logprobs 归一化**读出（$K=3$）。格：$3 \times 3 \times 4$ 种子，每格普通轨道加两条提示轨道；锚点测量：3 种选项排列 × 8 次复述。

**读数与复现要点（本实验踩过的五个坎，全部计入设计）**：其一，该模型是推理型——每次响应先消耗思维链 token（被提供商掩码为 `***`），之后才写答案；`max_tokens` 必须留足（1200），否则推理烧光预算、答案从未生成、`logprobs.content` 为空，官方 openai SDK 遇到此结构直接崩溃（`len(None)`），本实验改用 raw HTTP（httpx）直连 API 自行解析。其二，无信息提问绝不能只问"哪个最可能"——模型会输出 "None" 之类的拒绝词，丢弃后得到伪锚点，必须强制"你必须命名一个颜色"。其三，锚点测量必须覆盖选项排列（3 排列 × 8 次），否则位置偏置冒充锚点倾向；其四，世界参数齐备时数字信息完全盖过位置偏置（对照实验 9 个成功样本、零反例）——故轨道读数单排列即可；其五，温度 0 下同提示输出确定，但跨格的"完全重合"含确定性伪象（见结果四注 b）。

![图 5.B：真实 LLM 上的信念锚点与证据动力学验证](/figures/ch05_llm_evidence.png)

**图 5.B 三面板读法**——（a）锚点：三个选项排列下的零信息信念，全部接近顶点（位置偏置），全局平均 $\kappa_A = 1.285$；（b）证据吸收：每条曲线两根线，实线为 LLM 与真后验 $p^B_t$ 的 KL、虚线为与"$d' = 0.9$ 假后验"的 KL——虚线几乎全程更低；（c）相位差记忆衰减：中位距离 $d_t$ 在第三步即归零。

**结果一：无信息锚点 $A$ 不是均匀点。** 三个排列各自的信念为：Red/Blue/Black 排列 $(0.999, 0.001, 0)$，Black/Red/Blue 排列 $(0.125, 0.0001, 0.875)$，Blue/Black/Red 排列 $(0.0001, 1.000, 0)$。位置偏置约 87.5%–100%（唯一的"叛逃"是 Black/Red/Blue 排列中 1/8 次选择 Red）。全局均值 $A = (0.375, 0.334, 0.292)$，$\kappa_A = 1.285$。**$A$ 距均匀只有 1.285 的条件数，但它的组成不是均匀：是"选项位置规律 × 轻度词偏好"的混合。** 这是 §5.9/永霖公式"先验锚点 $A$"的实测位置——$A$ 的存在不是理论构型，它本身就是可以测量的先验。

**结果二：输出层信念呈尖峰态——信念贴单纯形边界运动。** 第 0 步（题面已给袋比例 $7/2/1$、尚无报告）时，KL$\left(p^B_0 \| p^{\text{LLM}}_0\right)$ 的中位数已达约 6.8–7.4 nats：即使题面明写非对称比例，模型给出的信念接近 one-hot 顶点（概率质量几乎尽倾于单色）。**模型对概率世界的信仰是决断式的**——不在 §5.7 所说的"连续不确定的内部"游走，而是一次跳到某个近顶点。对永霖公式的意义：真实推理的吸引子系统 $\approx$ 顶点；$A$ 不仅偏离 $A^*$，而且落在顶点区。

**结果三：证据强度吸收不对称——弱证据被放大，强证据被忠实。** 下表（中位 KL：LLM 信念与真后验的 KL / 与 $d'=0.9$ 假后验的 KL）：

| d | t=1 | t=2 | t=3 | t=4 | t=5 |
|---|---|---|---|---|---|
| 0.55（真后验 / $d'=0.9$ 后验） | 5.82 / 0.96 | 3.30 / 1.34 | 3.74 / 0.40 | 4.06 / 0.40 | — |
| 0.70 | 3.32 / 0.92 | 0.79 / 0.05 | 1.72 / 0.38 | 0.40 / 0.02 | 0.28 / 0.02 |
| 0.90 | 0.60 | 0.03 | 0.31 | 0.02 | — |

$d = 0.90$ 时全程单调下降（终局中位 0.02，模型几乎精确贴贝叶斯）；$d = 0.70$ 时从第 2 步起先贴 $d' = 0.9$ 假后验再贴真后验；$d = 0.55$ 时"与 $d' = 0.9$ 假后验的距离"平均好过与真后验约 8 倍。**模型把题面证据可信度当作接近固定的 $d' \approx 0.9$ 使用：弱证据端系统放大、强证据端精确。** 这与人类保守主义（Edwards 1968：低估证据）方向相反——可一句话概括：*LLM 的证据强度天花板 $\approx 0.9$，弱证据一律"不信题面、按最强使用"*。

**结果四：压缩与记忆——相位差半衰期约 2 步。** 同世界、同报告尾段（第 2–6 条相同），两条轨道唯一区别是第一条报告为红或蓝。该初始分叉的距离 $d_1 \equiv 1.414 = \sqrt{2}$（两个顶点之间，全体 36 格无一例外）；中位距离 $d_2$ 起为 0；单步压缩中位 0.07–0.20；半衰期 2.0–2.5 步。按 5.9 节的窗口公式 $-\ln\varepsilon / \ln k$（$\varepsilon = 0.1$）推算约 2.3 步——**与实测一致**。两个注：（a）偶发再分叉：12 格中 1 格在 $t=6$ 仍保持 >0.1 的距离，且整段 max 全程可为 $\sqrt{2}$——与 §5.A 面板 (d) 注明的"暂态先放大后收缩"互为镜像（先收缩、后重新放大），说明确定性模型内部仍有混沌式再分裂；（b）温度 0 下 $d_t = 0$ 的"重合"部分是确定性伪象，统计上我们只认读法："一单位相位差在两步内不再影响下一步选择"。

**与理论对表**（四行）：锚点 = §5.9 先验锚点 $A$ 的实测（$A \neq$ 均匀，$\kappa_A = 1.285$）；尖峰态 = 能量井近顶点（推理轨道在顶点区内运动）；弱证据超反应 = 证据吸收不对称（新测量：$d' \approx 0.9$ 天花板）；相位差半衰期 ≈ 2 步 = 有效推理窗口的实测常数，与闭式压缩界同数量级。**一个诚实的边界**：$\eta_{\max}$、$\theta_{\max}$ 等闭式量描述的是连续动力学的安全步长；真实 LLM 的"步长"是证据强度的函数（弱证据亦具 $d' \approx 0.9$ 等效强度），其运动更接近"大步长、短窗口、顶点间跳跃"。

*图 5.B 与源码：李籽溪（兔狲教授），2026。脚本同步维护于 `docs/public/scripts/yonglin_llm_exp.py` 与 `docs/public/scripts/compress_phase.py`。*
