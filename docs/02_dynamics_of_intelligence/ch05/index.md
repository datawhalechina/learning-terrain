...
这个广义勾股定理不是美学装饰。它给出 Bregman 投影“不增加与集合内点散度”的保证（广义勾股不等式）——这是 Bregman 几何自洽性的标志。但要注意它的边界：它本身不足以充当巴拿赫的入场券，因为 KL 不是度量。永霖极限的严格证明（§5.9）把角色拆开——KL 管能量与山形，欧氏度量管距离与完备。

:::info

**广义勾股定理：Bregman 投影为什么“降散度”——以及它为什么不能单独承载巴拿赫**
...
它给 Bregman 投影一个漂亮的推论：把 $r$ Bregman-投影到凸集 $S$ 上得到 $q$，则对任何 $p \in S$，$D_F(p\|q) \le D_F(p\|r)$——**投影不增加与集合内点的散度**（广义勾股不等式）。Bregman 几何的自洽性在这里：在弯曲的熵地形上，投影依然“降散度”。

但它不能单独充当巴拿赫的入场券。早期版本的永霖极限证明想直接拿它当“巴拿赫应用于 KL”的几何基础——这一步其实不成立：欧拉步里被投影的点是 $p - \eta\nabla E(p)$，不是 $p$ 本身；勾股不等式比较的是投影点与**被投影点**，给不出 $D_{\mathrm{KL}}(p_1\|q_2) \le D_{\mathrm{KL}}(p_1\|p_2)$。更根本地，KL 不是度量，“概率单纯形在 KL 下完备”这句话不成立，巴拿赫根本不能在 KL 上进场。所以 §5.9 换了承重梁：KL 管能量与山形（勾股结构保证能量语言自洽），切空间 Hessian 谱管 $\mu, L, \kappa$，欧氏度量管距离与完备。勾股定理还是美的——只是它不再一个人扛整座桥。
...
（注意这里 KL 的方向是 $p^*\|p_t$——用当前模型去“解释”真实数据的额外代价。这个方向的选择不是随意的：它让 Hessian $\operatorname{diag}(p_i^*/p_i^2)$ 把训练统计直接编码进谱界 $\mu, L$——“数据偏置决定地形”的几何来源。）

$E(p_t) \geq 0$，且 $E(p_t) = 0$ 当且仅当 $p_t = p^*$。这满足了李雅普诺夫函数（ch3）的两个基本条件——处处非负，在目标处归零。

**第二步：构造推理算子——以及一个必须现在做掉的更换。**

推理过程被建模为信念状态在单纯形上的离散时间演化。每一步，模型根据当前信念 $p_t$ 和输入 $x$，产生一个更新方向，然后沿这个方向迈出步长 $\eta$。在算法层，这个更新最自然的写法是 §5.7 的 softmax 投影：

$$p_{t+1} = \Phi^{\text{softmax}}_\eta(p_t) = \text{softmax}(\text{logit}(p_t) + \eta \cdot \text{update}(p_t, x))$$

这里的“update”方向可以从损失函数的梯度、注意力机制的输出、或者任何其他推理模块中产生。这是镜像下降（§5.6）在负熵镜下的实例——在对偶空间迈欧拉步，再用 softmax 投影回单纯形。

但把它直接当作定理分析的载体，会留下一个隐患：softmax 是 Bregman 投影，而下面的证明需要“投影不放大距离”（欧氏投影到闭凸集的非扩张性），softmax 没有这个性质。要证明的系统和被谈论的系统必须是同一个系统。

所以从这一步起，**理论动力系统**唯一定义为：

$$\Phi_\eta(p) = \Pi_{\mathcal X}\left(p - \eta\, P_T \nabla E(p)\right),$$

三个新对象各司其职——$\mathcal X$：单纯形内一个闭、凸、包含 $p^*$ 的区域（例如 $\mathcal X_\varepsilon = \{p : p_i \ge \varepsilon\}$），$\Pi_{\mathcal X}$ 是到它的欧氏正交投影；$T = \{v : \mathbf 1^\top v = 0\}$：单纯形的切空间；$P_T$：到 $T$ 的正交投影。为什么要它们，§5.9 会逐条说清——一句话预告：KL 的 Hessian 在边界爆炸，谱界需要一个远离边界的舞台；$\nabla E$ 的法向分量是规范自由度，要切掉。

这两个系统不是两个真理。$\Phi^{\text{softmax}}_\eta$ 与 $\Phi_\eta$ 是同一个连续极限——KL 梯度流 $\dot p = -P_T\nabla E(p)$——的两种一阶离散化：前者在 logit 坐标做乘性步，后者在原坐标做加性步。附录 5.A 的数值实验实现的正是 $\Phi_\eta$，本章的定理也为它成立；softmax 版的离散时间分析属于镜像下降框架（§5.6）的地盘。
...
永霖极限的枢纽在于：$\Phi_\eta$ 是一个**压缩映射**——而且这个压缩最终要落在能量上：每一步都让模型离 $p^*$ 的能量距离更小，且“变小”有一个固定的下界。

形式化地说，我们最终要证明存在一个常数 $\gamma \in (0, 1)$ 使得对于所有 $p \neq p^*$：
...
到达这个目标的路径分两步走（§5.9 的顺序）：先在某个**真正的度量**下建立压缩——欧氏范数，因为 KL 不是度量，巴拿赫不能站在 KL 上；再把欧氏压缩翻译回 KL 能量的指数衰减。

## 5.9 永霖极限 II：压缩性的完整证明

**第四步：换承重梁。**

上一节把目标说清楚了：存在 $\gamma \in (0, 1)$，使能量每步至少衰减为原来的 $\gamma$ 倍。现在兑现它。兑现之前，必须诚实交代早期版本的三处裂缝——不是修辞瑕疵，是承重结构的问题（本书的纪律：不成立的论证，要写明为什么不成立）：

1. **KL 不是度量**。它不对称、不满足三角不等式（§5.4 亲口说过）。因此“概率单纯形在 KL 散度下完备，故可调用巴拿赫”不成立——巴拿赫需要一个真正的度量空间。
2. **被证明的系统与被谈论的系统不一致**。§5.8 的算法层写的是 softmax 更新（Bregman 投影），早期版本的证明算的却是欧氏投影梯度 $\operatorname{proj}_{\mathcal P}(p - \eta\nabla E(p))$——两个不同的离散化，混用会让证明空转。
3. **“投影不增加散度”的那一步给不出**。早期版本用“投影不增加 KL”推出 $D_{\mathrm{KL}}(p_1\|q_2) \le D_{\mathrm{KL}}(p_1\|p_2)$。但欧拉步里被投影的点是 $p_2 - \eta\nabla E(p_2)$，不是 $p_2$；Bregman 勾股不等式比较的是投影点与**被投影点**，给不出与原来 $p_2$ 的比较。

修法不是打补丁，是换一根承重梁：**KL 管几何与能量；巴拿赫用切空间上的欧氏度量。** 每个数学对象回到它该站的位置——这并不削弱永霖极限，反而让结论更强：早期版本只能断言“离 $p^*$ 越来越近”，新版给出任意两点之间均匀的压缩。

**设定。** 取单纯形内一个闭、凸、包含 $p^*$ 的区域 $\mathcal X$——比如

$$\mathcal X_\varepsilon = \left\{ p \in \Delta^{K-1} : p_i \ge \varepsilon \right\}, \qquad 0 < \varepsilon \le \min_i p_i^*,$$

（$\varepsilon \le \min_i p_i^*$ 保证 $p^* \in \mathcal X_\varepsilon$；$\varepsilon \le 1/K$ 保证非空。）为什么要一个远离边界的区域？因为 KL 的 Hessian $\operatorname{diag}(p_i^*/p_i^2)$ 在边界处爆炸——谱界只有在紧致的内部区域才存在。这不是技术洁癖：§5.10 的 $\eta_{\max}(p) \to 0$（越靠边越不敢迈步）说的正是同一件事。记 $\Pi_{\mathcal X}$ 为到 $\mathcal X$ 的欧氏正交投影；由构造，$\Pi_{\mathcal X}$ 把一切点送回 $\mathcal X$，前向不变性免费获得。

切空间为

$$T = \left\{ v \in \mathbb R^K : \mathbf 1^\top v = 0 \right\},$$

记 $P_T$ 为到 $T$ 的正交投影。$\nabla E$ 的法向分量是规范自由度——沿法向移动不改变信念本身——切掉它，谱界才是干净的。

**理论动力系统**从此唯一定义为

$$\Phi_\eta(p) = \Pi_{\mathcal X}\left( p - \eta P_T \nabla E(p) \right),$$

其中 $E(p) = D_{\mathrm{KL}}(p^*\|p)$ 保持 §5.8 的定义不变。它与 §5.8 的 softmax 更新（镜像下降，§5.6）是同一个连续极限——KL 梯度流 $\dot p = -P_T\nabla E(p)$——的两种一阶离散化：前者在 logit 坐标做乘性步，后者在原坐标做加性步。附录 5.A 的数值实验实现的正是 $\Phi_\eta$；本章的定理为它成立。

**假设（切空间谱界）。** 在整个 $\mathcal X$ 上，限制到切空间的 Hessian 有统一谱界：

$$\mu I_T \preceq P_T \nabla^2 E(p) P_T \preceq L I_T, \qquad \forall p \in \mathcal X,$$

其中 $\mu, L > 0$（谱下界不超过谱上界，恒有 $\mu \le L$）。这就是 $\mu, L, \kappa$ 进入巴拿赫的方式：$\mu$ 是“最平方向也有下坡”的保证，$L$ 是“最陡方向没有悬崖”的保证。

**定理（永霖压缩定理）。** 在上述设定与谱假设下，对任意

$$0 < \eta < \frac{2\mu}{L^2},$$

算子 $\Phi_\eta$ 在 $(\mathcal X, \|\cdot\|_2)$ 上是压缩映射，压缩系数

$$\rho(\eta) = \sqrt{1 - 2\eta\mu + \eta^2 L^2} < 1.$$

**证明。**

*第一步：谱界翻译成两条不等式。* 在凸区域 $\mathcal X$ 上，谱下界即切方向的强凸性，谱上界即切方向的光滑性。对任意 $p, q \in \mathcal X$（注意 $p - q \in T$），由积分中值定理

$$\nabla E(p) - \nabla E(q) = \int_0^1 \nabla^2 E\big(q + s(p-q)\big)(p-q)\,ds,$$

代入谱界得到

$$\langle P_T(\nabla E(p) - \nabla E(q)),\, p - q \rangle = \langle \nabla E(p) - \nabla E(q),\, p - q \rangle \ge \mu\|p-q\|_2^2,$$

（等式处用了 $p-q \in T$ 与 $P_T$ 的对称性）

$$\|P_T(\nabla E(p) - \nabla E(q))\|_2 \le L\|p-q\|_2.$$

*第二步：投影不放大距离。* 欧氏投影到闭凸集是 non-expansive：

$$\|\Pi_{\mathcal X}(x) - \Pi_{\mathcal X}(y)\|_2 \le \|x-y\|_2,$$

（经典事实：往同一个凸集里“收”，两个投影点只会更近，不会更远。）于是

$$\|\Phi_\eta(p) - \Phi_\eta(q)\|_2^2 \le \big\|(p-q) - \eta P_T(\nabla E(p) - \nabla E(q))\big\|_2^2.$$

*第三步：展开。*

$$\begin{aligned}
\|\Phi_\eta(p) - \Phi_\eta(q)\|_2^2
&\le \|p-q\|_2^2 - 2\eta\langle P_T(\nabla E(p)-\nabla E(q)),\, p-q\rangle + \eta^2\|P_T(\nabla E(p)-\nabla E(q))\|_2^2\\
&\le \left(1 - 2\eta\mu + \eta^2 L^2\right)\|p-q\|_2^2.
\end{aligned}$$

强凸给了正贡献 $-2\eta\mu$，光滑给了负贡献 $\eta^2L^2$——**$2\mu - \eta L^2$ 是压缩的心脏**：这个差为正的条件正是步长上界 $\eta < 2\mu/L^2$。令 $\rho(\eta) = \sqrt{1-2\eta\mu+\eta^2L^2}$：当 $\eta \in (0, 2\mu/L^2)$ 时 $0 \le \rho(\eta) < 1$（非负性来自 $\mu \le L$：$\rho^2 \ge 1 - \mu^2/L^2 \ge 0$）。**啪。**

$$\|\Phi_\eta(p) - \Phi_\eta(q)\|_2 \le \rho(\eta)\,\|p-q\|_2$$

对**任意两个** $p, q \in \mathcal X$ 成立。这就是与早期版本的根本区别：早期版本只能比较 $p$ 与 $p^*$（李雅普诺夫式“离目标越来越近”）；这是全区域均匀的 Lipschitz 收缩——任何两点之间的距离都被压到原来的 $\rho$ 倍。证毕。

**第五步：巴拿赫堂堂正正进场。** $(\mathcal X, \|\cdot\|_2)$ 是 $\mathbb R^K$ 的闭子集，在欧氏度量下完备。巴拿赫不动点定理给出：

1. **存在唯一**不动点 $\bar p \in \mathcal X$，$\Phi_\eta(\bar p) = \bar p$；
2. 从任意 $p_0 \in \mathcal X$ 出发，迭代 $p_{t+1} = \Phi_\eta(p_t)$ 收敛到 $\bar p$；
3. $\|p_t - \bar p\|_2 \le \rho(\eta)^t \|p_0 - \bar p\|_2$。

KL 在这里不是度量——它不需要是。度量是欧氏范数；KL 的角色是能量。

**第六步：唯一不动点就是 $p^*$。** 对 $E(p) = D_{\mathrm{KL}}(p^*\|p)$，

$$\nabla_i E(p) = -\frac{p_i^*}{p_i},$$

在 $p = p^*$ 处 $\nabla E(p^*) = (-1, \ldots, -1)$——纯法向。于是 $P_T \nabla E(p^*) = 0$，进而

$$\Phi_\eta(p^*) = \Pi_{\mathcal X}(p^* - 0) = p^*.$$

$p^*$ 是不动点；巴拿赫说不动点唯一；故 $\bar p = p^*$。

**第七步：KL 回来——把欧氏压缩翻译成能量衰减。** KL 从未离场。它一回来就带着自己的 Hessian：

$$\nabla^2 E(p) = \operatorname{diag}\left(\frac{p_i^*}{p_i^2}\right),$$

这正是产生 $\mu, L, \kappa$ 的那个张量。在凸区域 $\mathcal X$ 上，由 Taylor 积分余项与谱界——线性项 $\langle \nabla E(p^*), p - p^* \rangle$ 因 $p - p^* \in T$ 且 $P_T\nabla E(p^*) = 0$ 而为零——

$$\frac{\mu}{2}\|p - p^*\|_2^2 \;\le\; D_{\mathrm{KL}}(p^*\|p) \;\le\; \frac{L}{2}\|p - p^*\|_2^2.$$

因此巴拿赫给出的欧氏指数收敛自动翻译成 KL 能量的指数衰减：

$$D_{\mathrm{KL}}(p^*\|p_t) \le \frac{L}{2}\,\rho^{2t}\,\|p_0 - p^*\|_2^2 \le \kappa\,\rho^{2t}\,D_{\mathrm{KL}}(p^*\|p_0), \qquad \kappa = \frac{L}{\mu}.$$

**术语纪律**：上面的双向等价不要叫“Pinsker 不等式的逆向形式”。Pinsker 给的是**下界** $D_{\mathrm{KL}} \ge \tfrac12\|\cdot\|_1^2$；反向控制需要“远离边界”这样的附加条件，它的来源是 Taylor 展开加 Hessian 谱界。早期版本那个叫法，删掉。

**推论（有效推理窗口）**。由 $\|p_t - p^*\|_2 \le \rho^t\|p_0 - p^*\|_2$，达到精度 $\|p_t - p^*\|_2 < \varepsilon$ 至多需要

$$t_{\mathrm{eff}}(\varepsilon) \le \left\lceil \frac{\ln\left(\|p_0 - p^*\|_2/\varepsilon\right)}{\ln(1/\rho)} \right\rceil$$

步。窗口长度由 $\rho$ 唯一控制——$\rho$ 越小，窗口越短。

**第八步（可选，更狠的一条路）：连巴拿赫都可以不要。**

**定理（永霖几何收敛定理）**。若存在 $\gamma \in (0,1)$ 使

$$D_{\mathrm{KL}}\big(p^*\|\Phi(p)\big) \le \gamma\, D_{\mathrm{KL}}(p^*\|p), \qquad \forall p \in \mathcal X,$$

则不动点唯一，且 $p_t \to p^*$——全程不需要巴拿赫，不需要度量完备。证明只有三行。第一行，迭代：$E(p_t) \le \gamma^t E(p_0)$。第二行，唯一性：若 $q$ 是不动点，$E(q) = E(\Phi(q)) \le \gamma E(q)$，因 $\gamma < 1$ 只能 $E(q) = 0$，即 $q = p^*$。第三行，收敛：Pinsker 不等式 $D_{\mathrm{KL}}(p^*\|p_t) \ge \tfrac12\|p_t - p^*\|_1^2$ 给出

$$\|p_t - p^*\|_1 \le \sqrt{2\gamma^t E(p_0)} \to 0.$$

（Pinsker 是 KL 控制总变差/$\ell_1$ 的标准结果——这一次它按自己的方向被使用。）两条定理的分工：**永霖压缩定理**给出衰减的动力学来源（切空间谱界 + 步长条件 ⇒ 欧氏压缩 ⇒ 巴拿赫）；**永霖几何收敛定理**给出衰减的逻辑后果（统一 KL 收缩 ⇒ 唯一极限，Pinsker 传递到 $\ell_1$）。当 $\kappa\rho^2 < 1$ 时（条件数不太坏、步长选得对），压缩定理直接满足几何收敛定理的假设（取 $\gamma = \kappa\rho^2$）；否则压缩定理仍给出无条件成立的 $\kappa\rho^{2t}$ 衰减界。
...
第一步，你走过了距离的 90%。第二步，你再走剩余距离的 90%。第三步……无论你从房间的哪个角落开始，你一定会无限接近正中心——因为每一步都把剩余距离压缩到原来的 10%（即压缩因子 $\rho = 0.1$）。你甚至不需要知道中心在哪——你只需要知道每一步都在向它靠近，并且靠近的比例是确定的。

这就是巴拿赫压缩映射原理的全部。它只要求三件事：

1. 你有一个“距离”的概念——在永霖极限中，这是信念空间里的欧氏距离 $\|\cdot\|_2$（在闭凸区域 $\mathcal X$ 上）。KL 散度在这里不充当距离——它充当能量：衡量你离谷底 $p^*$ 还有多高。
2. 你的映射每一步都在压缩距离——在永霖极限中，$\|\Phi_\eta(p) - \Phi_\eta(q)\|_2 \le \rho\|p-q\|_2$ 且 $\rho < 1$。
3. 空间是“完备的”——任何柯西序列都有极限。$\mathcal X$ 是 $\mathbb R^K$ 的闭子集，在欧氏度量下自动完备。

如果这三条成立，巴拿赫定理就保证：不动点唯一存在，且无论从哪出发，你都会到那里。**你不需要“希望”收敛——你证明了收敛。** 这就是永霖极限的数学心脏。

（一个诚实的备注：这个“房间”比喻里的距离是欧氏的。早期版本想直接拿 KL 散度当这个距离——但 KL 不是度量，房间的“完备性”在 KL 下也不成立。把度量换成欧氏、把 KL 留给能量，是本节换承重梁的全部内容。）

:::

## 5.10 永霖极限 III：η_max(p) 的推导与坐标不变性

压缩性成立的步长范围不是任意的。$\eta$ 必须足够小以保证压缩系数 $\rho(\eta) < 1$。但“足够小”是多小？这个值依赖于当前位置 $p$——因为概率单纯形不同区域的曲率不同。

**推导。** 压缩定理要求 $\eta < 2\mu / L^2$，其中 $\mu, L$ 是切空间 Hessian 在区域 $\mathcal X$ 上的谱极值（§5.9）。这两个常数不是神秘的数字——它们直接编码在 $\nabla^2 E(p)$ 中。

对于交叉熵损失 $E(p) = \mathbb{E}_D[-\log p_y]$（$q_i = P_D(y=i)$ 为训练标签边际；与 §5.8 的 $D_{\mathrm{KL}}(q\|p)$ 只差一个常数，梯度与 Hessian 完全相同），Hessian 在概率单纯形上的一般位置为：
...
在不动点 $p = q$ 处，谱极值就是该曲率张量的最小与最大特征值——对应最平坦与最陡峭的方向：
...
其中 $\kappa$ 是曲率各向异性（谱比/条件数）——$\max_i p_i$ 离 1 多远、$\min_i p_i$ 离 0 多远，即**离单纯形边缘的脆弱度**。读法：最高曲率 $L$ 告诉你“多陡”，$\kappa$ 告诉你“多各向异性”，安全步长被二者的乘积压缩。

**两个步长界的角色分工（重要）。** $\eta_{\max} = 2\mu/L^2 = 2/(L\kappa)$ 是**保守的全局压缩充分条件**：在含 $p^*$ 的闭凸区域上，步长落在这个界内，$\rho(\eta) < 1$，收敛由巴拿赫保证——它负责“保证不会炸”。而“什么时候开始局部失稳”是另一个问题，由 $p^*$ 处切空间 Jacobian 的**局部线性稳定阈值**回答：

$$\eta_{\text{crit}} = \frac{2}{\theta_{\max}}, \qquad \theta_{\max} = \text{最大特征值}\;\operatorname{diag}\left(\frac{1}{q_i}\right)\Big|_{\text{切空间}}$$

K = 3 时 $\theta_{\max}$ 是

$$3q_0q_1q_2\, \theta^2 - 2(q_0q_1+q_0q_2+q_1q_2)\, \theta + 1 = 0$$

的最大根。由于 $\theta_{\max} \leq L$，恒有 $\eta_{\text{crit}} \geq 2/L \geq 2\mu/L^2 = \eta_{\max}$——保守界恒不大于精确的局部阈值，两界在附录 5.A 的数值验证中同时可见：精确界（数值点贴线，中位误差 2.16%）与保守界（数值点恒在其上）。不要把 2.16% 的贴合归给 $\eta_{\max}$——那是 $\eta_{\text{crit}}$ 的成绩单。
...
**第二，各就各位，而非一肩双挑。** 信念空间有它自己的几何——KL 散度是负熵函数的 Bregman 散度，是信念空间的内禀几何。它适合当能量，不适合当度量——它不对称，不满足三角不等式。早期版本让 KL 同时当能量和度量，两个角色互相拖累，甚至把“KL 下完备”当成了事实。新版把角色拆开：KL 管能量与山形，切空间 Hessian 谱管 $\mu, L, \kappa$，欧氏度量管距离与完备。每个数学对象只干自己该干的事——这正是证明变严格的地方。

**第三，让参数有物理意义。** $\eta_{\max}(p)$ 不是经验调出来的“最佳学习率”，而是由单纯形的局部曲率决定的步长上界。$\rho(\eta)$ 不是神秘的“收敛速率超参数”，而是切空间谱界与步长的几何表达。$p^*$ 不是“训练集上的最优参数”，而是推理动力系统在欧氏度量下的唯一不动点——同时也是 KL 能量的谷底。每一个符号都对应着一个清晰的几何实体。
...
永霖极限将这整套几何语言推到了它的理论终点：从能量函数（KL 散度）到欧拉步推理算子，从切空间 Hessian 谱到欧氏压缩，从巴拿赫不动点到 KL 能量的指数衰减，从 $\eta_{\max}(p)$ 的坐标不变性到“推理必然收敛”的几何证明。它不只是一种训练技巧——它是信念空间中推理动力学的第一性原理。
...
3. Bregman 散度的广义勾股定理描述 Bregman 投影“不增加与集合内点散度”的性质。永霖极限的证明如今承载于切空间 Hessian 谱界与欧氏投影的非扩张性上（§5.9）。在什么条件下，切空间谱界在深度网络的非凸损失地形上仍然成立——至少是近似成立？
...
8. 巴拿赫不动点定理要求一个真正的度量。早期版本曾想用 KL 散度充当度量，但 KL 不对称、不满足三角不等式——本书已将巴拿赫的舞台换成闭凸区域上的欧氏度量（§5.9）。一个自然的后续问题：在 Hellinger 距离或全变差距离下，能否得到比 $\rho(\eta)$ 更优的压缩常数？
...
**切空间上的 Hessian 谱——永霖极限的几何心脏——在非线性、高维、随机梯度噪声污染的深度网络中，以什么形式存活？**
